        ...
        <div className="min-h-screen max-w-6xl mx-auto " id="shop">
                    ...
                    <div className="flex gap-6 sm:gap-8 pr-4 md:pr-6">
                        {categoriesLoading
                            ? Array.from({ length: 8 }).map((_, i) => (
                                <div
                                    key={i}
                                    className="flex flex-col items-center w-20 animate-pulse"
            ...
            <section className="py-12 group">
                ...
                <div className="relative flex items-center">
                    <button
                        ...
                        className="p-3 rounded-full bg-white/80 backdrop-blur-sm shadow-lg hover:bg-white hover:shadow-xl transition-all duration-300 z-10 ml-2 md:ml-0 absolute left-0 hover:scale-110 transform-gpu border border-gray-100"
                    >
                    ...
                    <div
                        ...
                        className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory px-4 py-2 scroll-smooth"
                    >
                        {productsLoading
                            ? Array.from({ length: 5 }).map((_, index) => (
                                <div key={index} className="snap-start flex-shrink-0 w-[85%] sm:w-[45%] md:w-[30%] lg:w-[280px]">
                                    <ProductCardSkeleton index={index} />
                                </div>
                            ))
                            : newArrivals.map((product, index) => (
                                <div key={product.id} className="snap-start flex-shrink-0 w-[85%] sm:w-[45%] md:w-[30%] lg:w-[280px]">
                    ...
                    <button
                        ...
                        className="p-3 rounded-full bg-white/80 backdrop-blur-sm shadow-lg hover:bg-white hover:shadow-xl transition-all duration-300 z-10 mr-2 md:mr-0 absolute right-0 hover:scale-110 transform-gpu border border-gray-100"
                    >
            ...
            <section className="px-4 md:px-6 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {[
                        { title: "Featured Products", products: featuredProducts, color: "primary" },
                        { title: "Most-viewed Items", products: mostViewed, color: "slate-600" },
                        { title: "We Think You'll Love", products: weThinkYoullLove, color: "slate-700" },
                    ].map((section, sectionIndex) => (
                        <div key={section.title} className="group" style={{ animationDelay: `${sectionIndex * 200}ms` }}>
                            <div className="mb-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-2">{section.title}</h3>
                                <div
                                    className={`w-12 h-1 rounded-full transition-all duration-700 ease-out group-hover:w-[80%] ${section.color === "primary"
                                            ? "bg-primary"
                                            : section.color === "slate-600"
                                                ? "bg-slate-600"
                                                : "bg-slate-700"
                                        }`}
                                ></div>
                            ...
                            <div className="space-y-4">
                                {productsLoading
                                    ? Array.from({ length: 3 }).map((_, i) => (
                                        <div
                                            key={i}
                                            className="flex gap-4 p-3 animate-pulse rounded-xl"
                                            style={{ animationDelay: `${i * 100}ms` }}
                                        >
                                    ...
                                    : section.products.map((product, index) => (
                                        <Link
                                            ...
                                            key={product.id}
                                            className="flex gap-4 p-3 rounded-xl hover:bg-white hover:shadow-lg transition-all duration-300 cursor-pointer transform-gpu hover:-translate-y-1"
                                            style={{ animationDelay: `${index * 100}ms` }}
                                        >
                                            <div className="relative overflow-hidden rounded-xl">
                                                <img
                                                    src={product.images?.[0]?.imageUrl || "https://placehold.co/150x150/e2e8f0/cccccc?text=Img"}
                                                    alt={product.name}
                                                    className="w-16 h-16 object-cover flex-shrink-0 bg-gray-100 transition-transformduration-300 hover:scale-110"
                                                />
                                                <div className="absolute inset-0 bg-primary/20 opacity-0 hover:opacity-100 transition-opacity duration-300"></div>
                                            </div>
                                            <div className="flex-1 space-y-1">
                                                <h4 className="text-sm font-medium text-gray-900 line-clamp-2 hover:text-primary transition-colors duration-300">
                                                    {product.name}
                                                </h4>
