                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div>
                                ...
                                {Object.entries(variantOptions).map(([key, values]) => (
                                    <div key={key}>
                                        <h3 className="text-sm font-medium text-gray-900 capitalize">
                                            {key}: <span className="text-gray-600 font-normal">{userSelections[key] || ''}</span>
                                        </h3>
                                            ...
                                            {values.map(value => {
                                                ...
                                                if (colorHex) {
                                                    return (
                                                        <button
                                                            ...
                                                            className={`w-8 h-8 rounded-full border-2 transition-all ${isSelected ? 'ring-2 ring-primary ring-offset-2' : 'border-gray-200 hover:border-primary'}`}
                                                ...
                                                return (
                                                    <button
                                                        key={value}
                                                        onClick={() => handleSelection(key, value)}
                                                        className={`px-4 py-2 border rounded-md text-sm font-medium transition-colors ${isSelected ? 'bg-primary text-white border-primary' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
                                                    >
                                                        {value}
                                                    </button>
                                                );
                            ...
                            <div className="flex items-center gap-4 mt-6">
                                ...
                                <button onClick={handleAddToCart} disabled={!selectedVariant || cartLoading} className="flex-grow bg-primary text-white py-3 rounded-md font-semibold hover:bg-fblack transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">
                                    {cartLoading ? 'Adding...' : (!selectedVariant ? 'Select Options' : 'Add to Cart')}
                                </button>
                            </div>
