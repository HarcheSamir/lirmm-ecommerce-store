...
export default function ProductCard({ product, index = 0 }) {
    ...
    const isProductOutOfStock = product.variants?.every(v => v.stockQuantity === 0);
    const baseButtonText = !isSimpleColorVariantProduct ? "Select Options" : "Add to Cart";
    const buttonText = isProductOutOfStock ? "Out of Stock" : baseButtonText;
    const isButtonDisabled = isProductOutOfStock || (isSimpleColorVariantProduct && !selectedVariantId);
                    ...
                    {isSimpleColorVariantProduct && (
                        <div className="absolute bottom-4 left-4 flex gap-2 opacity-0 translate-y-8 group-hover/card:opacity-100 group-hover/card:translate-y-0 transition-all duration-500 ease-out">
                            {product.variants.map((variant, idx) => (
                                ...
                                <button
                                    key={variant.id}
                                    title={variant.attributes.Color || 'Variant'}
                                    disabled={variant.stockQuantity === 0}
                                    className={`relative w-7 h-7 rounded-full border-2 shadow-lg transition-all duration-300 hover:scale-125 transform-gpu ${selectedVariantId === variant.id ? "ring-2 ring-offset-2 ring-primary border-white" : "border-white/60"} ${variant.stockQuantity === 0 ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                    style={{ backgroundColor: variant.attributes.colorHex, animationDelay: `${idx * 100}ms` }}
                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedVariantId(variant.id); }}>
                                    {variant.stockQuantity === 0 &&
                                        <div className="absolute inset-0 flex items-center justify-center bg-white/40 rounded-full" title={`${Object.entries(variant.attributes).filter(([key]) => key !== "colorHex").map(([_, value]) => value).join(", ")} (Out of Stock)`}>
                                            <svg className="w-4 h-4 text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                        </div>
                                    }
                                </button>
                                // --- END: SURGICAL MODIFICATION ---
