...
export default function ProductCard({ product, index = 0 }) {
    ...
    const categoryName = product.category_names?.[0] || product.categories?.[0]?.category?.name || "Uncategorized";
            ...
            <Link to={`/products/${product.id}`} className="block">
                <div className="relative h-64 overflow-hidden bg-gray-50">
                    ...
                    <div className="absolute top-4 -right-16 flex flex-col gap-3 transition-all duration-500 ease-out group-hover/card:right-4">
                        ...
                        <button onClick={(e) => { e.preventDefault(); toast.info("Quick view coming soon!"); }} className="w-10 h-10bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white transition-all duration-300 shadow-lg hover:shadow-primary/25 hover:scale-110 transform-gpu"><FiEye size={16} /></button>
                    </div>
