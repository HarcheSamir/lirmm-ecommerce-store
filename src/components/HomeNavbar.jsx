...
export default function HomeNavbar() {
    ...
    // State now holds the full category object and its position
    ...
    const handleMouseEnter = (event, category) => {
        if (category.isLeaf) return; // Don't open menu for leaf categories
    ...
    return (
        // The header is now the positioning context for the absolute menu
        <header ref={headerRef} className="w-full font-sans text-gray-700 bg-white relative">
            {/* Top navigation bar */}
            ...
            {/* THIS IS THE CRITICAL CHANGE: MegaMenu is rendered here, as a sibling to the nav, not a child. */}
            {activeMenu.category && (
                <MegaMenu 
